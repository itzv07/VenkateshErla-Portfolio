...
export const AIRecruiterChatbot: React.FC<AIRecruiterChatbotProps> = ({
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const assistantMsg: ChatMessage = {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: data.reply || "Venkatesh Erla is a Software Engineer with expertise in Java Spring Boot, AI/ML, and ServiceNow development. Feel free to reach out to him directly at venkatesherla21@gmail.com!",
        timestamp: new Date()
      };
      ...
      const fallbackMsg: ChatMessage = {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: "Venkatesh Erla is an accomplished Software Engineer specializing in Java Spring Boot backend, AI/ML, and ServiceNow development (CAD & CSA certified). Please feel free to email him directly at venkatesherla21@gmail.com!",
        timestamp: new Date()
      };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div
        className={`relative w-full max-w-2xl h-[85vh] max-h-[700px] rounded-3xl border flex flex-col overflow-hidden shadow-2xl ${
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                }`}>AI Recruiter Assistant</h3>
              </div>
              <p className={`text-[11px] ${
                theme === 'dark' ? 'text-zinc-400' : 'text-zinc-600'
              }`}>Trained on Venkatesh Erla's Resume & Achievements</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4">
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`group relative max-w-[85%] p-4 rounded-2xl text-xs leading-relaxed space-y-1 ${
                msg.role === 'user'
          ...
          {isLoading && (
            <div className="flex gap-3 items-center text-xs text-zinc-400">
              ...
              <div className="p-3 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center gap-2">
                ...
                <span className="font-mono text-[11px] text-zinc-400">Analyzing candidate background...</span>
              </div>
            </div>
          )}
        ...
        {messages.length <= 2 && (
          <div className={`px-6 py-2 border-t overflow-x-auto flex gap-2 no-scrollbar ${
            theme === 'dark' ? 'border-zinc-800/40 bg-zinc-950/40' : 'border-zinc-200 bg-zinc-50'
          }`}>
            {RECRUITER_SAMPLE_PROMPTS.map((prompt, idx) => (
              <button
                key={idx}
                onClick={() => handleSendMessage(prompt)}
                className={`shrink-0 px-3 py-1.5 rounded-full border text-[11px] font-medium transition-all ${
                  theme === 'dark'
        ...
        <form
          ...
          className={`p-4 border-t flex items-center gap-3 ${
            theme === 'dark' ? 'border-zinc-800/60 bg-zinc-950/80' : 'border-zinc-200 bg-zinc-50'
          }`}
        >
          <input
            type="text"
            ...
            disabled={isLoading}
            className={`flex-1 px-4 py-2.5 rounded-xl text-xs font-medium border outline-none transition-all ${
              theme === 'dark'
